...
class ModelGenerator extends BaseCommand
{
     ...
     * @var string
     */
    protected $description = 'Generates a new model file in module.';
    ...
    {   
        ...
        {
            $fSave = [];
            foreach($getMigration['fields'] as $k => $v)
            {
                if(!isset($v['join']) && $setText=='' && $k!='id')
                {
                    if (!str_contains($k, 'parent_id')) { 
                        $setText = $k;
                        $nestedText = $k;
                    }
                }
                if(isset($v['join']))
                {
                    ...
                    {
                        foreach ($v['jointable'] as $x) {
                            $setSelect[] = "{$v['join']}.{$x}";
                        }
                    }
                    $select .= "\n\t\t\t\t\t->select('".implode(',',$setSelect)."')";
                    $select .= "\n\t\t\t\t\t->join('{$v['join']}',\$this->table.'.{$k}={$v['join']}.id','LEFT')";
                }
