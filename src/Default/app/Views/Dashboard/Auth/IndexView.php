	...
	<body>
		...
		<script type="text/javascript">
			...
			grecaptcha.ready(function() {
			    grecaptcha.execute('<?=env('reCAPTCHA.siteKey')?>', {action: 'submit'}).then(function(token) {
			        // Kirim token ke input hidden
		    ...
		    elFormData.addEventListener('submit', async function (e) {
		        ...
		        if(valid)
		        {
		        	let post = await fetch(`/api/auth/signin`,{
		        		method : 'POST',
        			body:`username=${document.querySelector('[name=username]').value}&password=${document.querySelector('[name=password]').value}&recaptcha_token=${recaptchaToken}`,
		        		headers: {
