...
function input_hidden($a, $b, $c = '', $d = '')
{
    if(!is_array($c)){
        ...
        if(is_array($b)){
            return '<input type="hidden" name="' . $a . '" value=\'' . $b[0] . '\' class="form-control' . $c . '"' . $d . '>' . "\n";
        }
        else{
    ...
    if(!is_array($d)){
        ...
        foreach ($b as $key => $val) {
            if(is_array($val))
            {
                ...
                {
                    $sel = (in_array($key2, $c)) ? ' selected="selected"' : '';
                    $data .= "\t\t\t\t\t\t<option value=\"" . $key2 . "\"" . $sel . ">" . (string) $val2 . "</option>\n";
                }
                $data .= '</optgroup>';
            }
            else
